...
type RustDocsNavProps = {
  docs: DocEntry[];
  activeSlug: string;
};

const SECTION_ORDER = ["Start", "Subsystems", "UI", "Reference"];

export default function RustDocsNav({ docs, activeSlug }: RustDocsNavProps) {
  ...
  return (
    <div className="hidden lg:block sticky top-16">
      <div className="surface rounded-2xl sm:rounded-3xl p-4 sm:p-5">
        ...
        <div className="tag-rust inline-flex items-center rounded-full px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-xs uppercase tracking-[0.2em] mt-2 sm:mt-3">
          Rust Framework
        </div>
        <div className="mt-1.5 sm:mt-2 text-xl sm:text-2xl font-semibold text-[color:var(--ink)]">indusagi</div>
        ...
        <div className="mt-4 sm:mt-5">
          <input
            ...
            className="w-full rounded-xl border border-[color:var(--border)] bg-[rgba(30,30,30,0.8)] px-3 py-2 text-sm outline-none focus:border-[#fbbf24]"
          />
        </div>
                    ...
                    return (
                      <Link
                        ...
                        className={`rounded-lg px-2.5 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm transition-colors ${
                          isActive
                            ? "bg-[rgba(251,191,36,0.15)] text-[#fbbf24] font-medium"
