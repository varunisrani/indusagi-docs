...
type PythonDocsNavProps = {
  docs: DocEntry[];
  activeSlug: string;
};

const SECTION_ORDER = ["Start", "Facades", "Subsystems", "UI", "Reference"];

export default function PythonDocsNav({ docs, activeSlug }: PythonDocsNavProps) {
  ...
  return (
    <div className="hidden lg:block sticky top-16">
      <div className="surface rounded-2xl sm:rounded-3xl p-4 sm:p-5">
        ...
        <div className="tag-python inline-flex items-center rounded-full px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-xs uppercase tracking-[0.2em] mt-2 sm:mt-3">
          Python
        </div>
        <div className="mt-1.5 sm:mt-2 text-xl sm:text-2xl font-semibold text-[color:var(--ink)]">Indusagi for Python</div>
                    ...
                    return (
                      <Link
                        ...
                        className={`rounded-lg px-2.5 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm transition-colors ${
                          isActive
                            ? "bg-[rgba(52,211,153,0.15)] text-[#34d399] font-medium"
