...
type RustCliDocsNavProps = {
  docs: DocEntry[];
  activeSlug: string;
};

const SECTION_ORDER = ["Start", "Console", "Subsystems", "Configuration", "Reference"];

export default function RustCliDocsNav({ docs, activeSlug }: RustCliDocsNavProps) {
  ...
  return (
    <div className="hidden lg:block sticky top-16">
      <div className="surface rounded-2xl sm:rounded-3xl p-4 sm:p-5">
        ...
        <div className="tag-rust-cli inline-flex items-center rounded-full px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-xs uppercase tracking-[0.2em] mt-2 sm:mt-3">
          Rust Agent
        </div>
        <div className="mt-1.5 sm:mt-2 text-xl sm:text-2xl font-semibold text-[color:var(--ink)]">induscode</div>
        <div className="mt-0.5 sm:mt-1 text-xs sm:text-sm text-[color:var(--muted)]">
          Rust coding-agent CLI
        </div>
        <div className="mt-4 sm:mt-5">
          <input
            ...
            className="w-full rounded-xl border border-[color:var(--border)] bg-[rgba(30,30,30,0.8)] px-3 py-2 text-sm outline-none focus:border-[#f43f5e]"
          />
        </div>
        <div className="mt-4 sm:mt-6 max-h-[50vh] sm:max-h-[60vh] lg:max-h-[68vh] overflow-y-auto pr-1">
