...
import RustCliDocsNav from "./RustCliDocsNav";
import { Header } from "../../_components/Header";

type RustCliDocsShellProps = {
  docs: DocEntry[];
  doc: DocContent;
};

export default function RustCliDocsShell({ docs, doc }: RustCliDocsShellProps) {
        ...
        <div className="mx-auto max-w-7xl px-3 sm:px-4">
          <div className="grid grid-cols-1 gap-6 sm:gap-8 lg:gap-10 lg:grid-cols-[260px_1fr] xl:grid-cols-[260px_1fr_220px]">
            ...
            <aside className="hidden lg:block lg:pt-2">
              <RustCliDocsNav docs={docs} activeSlug={doc.slug} />
            </aside>
            ...
            <main className="min-w-0">
              <div className="surface-strong rounded-2xl sm:rounded-[28px] px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-10">
                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                  <span className="tag-rust-cli rounded-full px-2.5 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-xs uppercase tracking-[0.15em] sm:tracking-[0.22em]">
                    {doc.section}
                  </span>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {toc.map((item) => (
                      <a
                        key={item.id}
                        href={`#${item.id}`}
                        className="inline-block rounded-lg bg-[rgba(244,63,94,0.1)] px-2.5 py-1 text-xs text-[color:var(--ink)] hover:bg-[rgba(244,63,94,0.2)] transition-colors"
                      >
                        {item.text}
                      </a>
                    ))}
              ...
              <div className="sticky top-20 space-y-4">
                ...
                <div className="surface rounded-2xl p-4">
                  ...
                  <div className="mt-3 text-sm text-[color:var(--muted)]">
                    induscode (Rust) is the coding agent compiled to a single native binary. It builds on the
                    Rust indusagi framework — see the Rust framework docs for the layers underneath.
                  </div>
                </div>
              </div>
            </aside>
